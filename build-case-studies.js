...
const scenarioLabel = 'Illustrative scenario';
...
const parseFrontmatter = (source, fileName) => {
  ...
  return {...data, topic: data.topic || 'Operations', cta: data.cta || 'View the scenario', disclosure: data.disclosure || 'This illustrative scenario is hypothetical and not based on a specific client or completed engagement.', body: match[2]};
};
...
const pageHead = (entry, isIndex = false) => {
  ...
  const title = isIndex ? 'Case Studies | Bates Operations' : `${entry.title} | Bates Operations`;
  const description = isIndex
    ? 'Operational scenarios for STR/MTR teams covering dispatch handoffs, vendor exceptions, and scaling coordination.'
    : entry.summary;
...
const footer = `<footer class="site-footer"><div class="container footer-inner"><span>Bates Operations</span><span class="footer-links"><a href="/">Home</a><a href="/about">About</a><a href="/diagnosis">Diagnosis</a><a href="/field-notes">Field notes</a><a href="/case-studies">Case studies</a><a href="/pricing">Pricing</a><a href="/contact">Contact</a></span></div></footer>`;

const card = (entry) => `<article class="article-card case-study-card"><div class="case-study-card-meta"><span class="topic-tag">${escapeHtml(entry.topic)}</span><span class="scenario-badge">${scenarioLabel}</span></div><h2><a href="/case-studies/${entry.slug}">${escapeHtml(entry.title)}</a></h2><p>${escapeHtml(entry.summary)}</p><p><a class="button secondary" href="/case-studies/${entry.slug}">${escapeHtml(entry.cta)}</a></p></article>`;
...
<section class="page-hero"><div class="container"><nav class="breadcrumbs" aria-label="Breadcrumb"><ol><li><a href="/">Home</a></li><li><a href="/case-studies">Case studies</a></li><li aria-current="page">${escapeHtml(entry.title)}</li></ol></nav><div class="case-study-card-meta"><span class="topic-tag">${escapeHtml(entry.topic)}</span></div><h1>${escapeHtml(entry.title)}</h1><p>${escapeHtml(entry.summary)}</p></div></section>
<article class="section"><div class="container"><p class="case-study-disclosure">${escapeHtml(entry.disclosure)}</p><div class="case-study-layout"><div class="content-copy article-body">${renderMarkdown(entry.body)}</div>${renderDiagram(entry)}</div><p class="case-study-actions"><a class="button primary" href="/diagnosis">Start the Operational Leak Diagnosis</a> <a class="button secondary" href="/contact">Ask a question</a></p></div></article>
<section class="section related-section" aria-labelledby="case-study-next"><div class="container"><div class="section-heading"><div><p class="label">Continue exploring</p><h2 id="case-study-next">More operating scenarios</h2></div><p>Explore another category of handoff, exception, or scaling friction the diagnosis can examine.</p></div><div class="article-grid case-study-grid">${readCases().filter((candidate) => candidate.slug !== entry.slug).map(card).join('')}</div></div></section>
</main>${footer}</div><script src="/site.js" defer></script></body></html>`;

const renderIndex = (entries) => `${pageHead({}, true)}
<body><div class="site-shell">${header}<main>
<section class="page-hero"><div class="container"><nav class="breadcrumbs" aria-label="Breadcrumb"><ol><li><a href="/">Home</a></li><li aria-current="page">Case studies</li></ol></nav><p class="label">How the framework applies</p><h1>How the framework applies</h1><p>Example scenarios across dispatch handoffs, vendor exceptions, and scaling coordination as a portfolio grows.</p><p class="case-study-index-disclosure">These are illustrative scenarios, not descriptions of specific client work.</p></div></section>
<section class="section"><div class="container"><div class="article-grid case-study-grid">${entries.map(card).join('')}</div><p><a class="button primary" href="/diagnosis">Start the Operational Leak Diagnosis</a></p></div></section>
</main>${footer}</div><script src="/site.js" defer></script></body></html>`;
